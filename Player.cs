...
public class Player
{
    ...
    public Player()
    {
        this._X = AsciiEngine.Width / 2 - this.Width / 2;
    }
    void Hide()
    {
    ...
    {
        this.Hide();
        this._Y = AsciiEngine.BottomEdge;
        if (this._X < AsciiEngine.LeftEdge) { this._Direction = 1; }
        if (this._X + this.Width > AsciiEngine.RightEdge) { this._Direction = -1; }
    ...
    {
        this._missiles.Sprites.Add(new AsciiEngine.Sprite('|', this._X + this.Width / 2, this._Y, 0, -1, this._Y));
    }
